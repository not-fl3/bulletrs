...
pub enum CapsuleShape {
    CapsuleX(sys::btCapsuleShapeX),
    CapsuleY(sys::btCapsuleShape),
    CapsuleZ(sys::btCapsuleShapeZ),
}
impl CapsuleShape {
    pub(crate) fn as_ptr(&self) -> *mut sys::btCollisionShape {
        match self {
            &CapsuleShape::CapsuleX(ref shape) => shape as *const _ as *mut _,
            &CapsuleShape::CapsuleY(ref shape) => shape as *const _ as *mut _,
            &CapsuleShape::CapsuleZ(ref shape) => shape as *const _ as *mut _,
        }
    }
}
...
impl Shape {
    ...
    pub fn new_capsule(radius: f64, height: f64, axis: CapsuleAxis) -> Shape {
        match axis {
            CapsuleAxis::X => Shape::Capsule(CapsuleShape::CapsuleX(
                unsafe { sys::btCapsuleShapeX::new(radius, height) },
            )),
            CapsuleAxis::Y => Shape::Capsule(CapsuleShape::CapsuleY(
                unsafe { sys::btCapsuleShape::new1(radius, height) },
            )),
            CapsuleAxis::Z => Shape::Capsule(CapsuleShape::CapsuleZ(
                unsafe { sys::btCapsuleShapeZ::new(radius, height) },
            )),
        }
    }
    ...
    pub fn new_convex_hull<T: Into<Vector3<f64>> + Clone>(vertices: &[T]) -> Shape {
        ...
        for vertex in vertices.iter() {
            ...
            unsafe {
                shape.addPoint(&vertex as *const _ as *const _, true);
            }
        }
        ...
        T1: Into<Vector4<f64>>,
    {
        ...
        for (shape, position, orientation) in shapes.into_iter() {
            ...
            unsafe {
                compound_shape.addChildShape(&transform as *const _, shape_box.as_ptr());
            }
    ...
    pub fn calculate_local_inertia(&self, mass: f64) -> Vector3<f64> {
        ...
        match self {
            &Shape::Sphere(ref sphere) => unsafe {
            ...
            &Shape::Capsule(ref shape) => unsafe {
            ...
            &Shape::ConvexHull(ref shape) => unsafe {
            ...
            &Shape::Compound { ref shape, .. } => unsafe {
